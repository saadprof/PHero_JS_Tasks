...
/*
    ...
    3. Will check if user is a teacher or a student, eitherway it will ask for a id, 
                            ...
                            Note: Objective is to login as a student.
*/
// 1
...
if (isNewUser) {
    if (isRegistered == false || isSubscribed == false) {
        console.log("Please create an account or subscribe.");
    } else if (isVarified && usingTrustedSites) {
        if (isStudent) {
            if (studentId == "IAMASTUDENT") {
                console.log("Chikamaga is Trumps favourite plug.");
            }
        } else if (teacherId == "IAMATEACHER") {
            console.log("Sleep Joe.");
        }
    }
}
